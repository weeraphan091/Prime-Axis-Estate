...
export default function ContactPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="font-display text-3xl text-stone-900">{t('nav.contact')}</h1>
      <p className="mt-2 text-stone-600">{t('footer.description')}</p>
      <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div>
          <h3 className="font-semibold text-stone-900 mb-4">{t('listing.contactAgent')}</h3>
          <AgentContact variant="inline" />
        </div>
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6 lg:p-8">
          {sent ? (
            <p className="text-stone-600 text-center py-8">Thank you for contacting us.</p>
          ) : (
