...
export function ChatButtons({ size = 'md', lineLabel, whatsappLabel, className = '' }: {
  size?: Size
  lineLabel?: string
  whatsappLabel?: string
  className?: string
}) {
  const { getLineUrl, getWhatsAppUrl } = useContact()
  ...
  return (
    <div className={`flex flex-wrap gap-3 ${className}`}>
      <a
        ...
        className={`inline-flex items-center gap-2 ${pad} bg-[#06C755] text-white rounded-lg font-semibold hover:bg-[#05b34d] transition`}
      >
        {lineSvg}
        {lineLabel ?? 'LINE'}
      </a>
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className={`inline-flex items-center gap-2 ${pad} bg-[#25D366] text-white rounded-lg font-semibold hover:bg-[#20bd5a] transition`}
      >
        {whatsappSvg}
...
export function ChatButtonsBlock({ lineLabel, whatsappLabel }: { lineLabel?: string; whatsappLabel?: string }) {
  const { getLineUrl, getWhatsAppUrl } = useContact()
